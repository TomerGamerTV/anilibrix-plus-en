<template>
  <div ref="settings">
    ...
    <div class="pa-4 caption grey--text">
      <div class="body-1">{{ $t('settings.player.title') }}</div>
      <div>{{ $t('settings.player.subtitle') }}</div>
    </div>

    <!-- Torrents -->
    <v-card>
      <v-card-subtitle class="pb-0 font-weight-bold">{{ $t('settings.player.torrentsTitle') }}</v-card-subtitle>
      <v-card-text class="mt-2 caption">
        <div class="pb-2">{{ $t('settings.player.torrentsDescription1') }}</div>
        <div>{{ $t('settings.player.torrentsDescription2') }}</div>
      </v-card-text>
      <v-list-item dense @click="_setTorrentsProcess(!_torrents_process)">
        <v-list-item-title>{{ $t('settings.player.playTorrents') }}</v-list-item-title>
        <v-list-item-action class="mr-2">
          <v-switch :input-value="_torrents_process" @change="_setTorrentsProcess"/>
        </v-list-item-action>
      </v-list-item>
      <v-card-text class="caption">
        {{ $t('settings.player.torrentsWarning') }}
      </v-card-text>
    </v-card>
    ...
    <v-card class="mt-2">
      <v-list-item dense @click="_setAutoplayNext(!_autoplay_next)">
        <v-list-item-title>{{ $t('settings.player.autoplayNextEpisode') }}</v-list-item-title>
        <v-list-item-action class="mr-2">
          <v-switch :input-value="_autoplay_next" @change="_setAutoplayNext"/>
        </v-list-item-action>
      </v-list-item>
      <v-card-text class="pt-2 caption">
        {{ $t('settings.player.autoplayNextEpisodeDescription') }}
      </v-card-text>
    </v-card>
    ...
    <v-card class="mt-2">
      <v-card-text class="pb-2 caption">
        <div>{{ $t('settings.player.videoBufferTitle') }}</div>
        <div>{{ $t('settings.player.videoBufferDescription') }}</div>
      </v-card-text>
      <v-card-text>
        <v-text-field
          outlined
          ...
          class="mb-2"
          type="number"
          :label="$t('settings.player.videoBufferLabel')"
      ...
      <v-card-text class="pt-0 caption">
        <div>{{ $t('settings.player.videoBufferHint') }}</div>
      </v-card-text>
    </v-card>
    ...
    <v-card class="mt-2">
      <v-list-item dense @click="_setAutoSkip(!_auto_opening_skip)">
        <v-list-item-title>{{ $t('settings.player.autoSkipOpening') }}</v-list-item-title>
        <v-list-item-action class="mr-2">
      ...
      <v-card-text class="pt-2 caption">
        <div>
          {{ $t('settings.player.autoSkipOpeningDescription') }}
        </div>
      </v-card-text>
    </v-card>

    <v-card>
      <v-card-text class="caption">{{ $t('settings.player.autoSkipHotKey') }}</v-card-text>
      <v-card-text>
        <v-text-field
          outlined
          hide-details
          class="mb-2"
          type="text"
          :label="$t('common.hotKeyPlaceholder')"
          clearable
    ...
    <v-card class="mt-2">
      <v-list-item dense @click="_setOpeningSkipButton(!_opening_skip_button)">
        <v-list-item-title>{{ $t('settings.player.skipOpeningButton') }}</v-list-item-title>
        <v-list-item-action class="mr-2">
          <v-switch :input-value="_opening_skip_button" @change="_setOpeningSkipButton"/>
        </v-list-item-action>
      </v-list-item>
      <v-card-text class="pt-2 caption">
        <div>{{ $t('settings.player.skipOpeningButtonDescription1') }}</div>
        <div>{{ $t('settings.player.skipOpeningButtonDescription2') }}</div>
      </v-card-text>
    </v-card>
    <v-divider/>

    <v-card>
      <v-card-text class="caption">{{ $t('settings.player.skipOpeningButtonHotKey') }}</v-card-text>
      <v-card-text>
        <v-text-field
          outlined
          hide-details
    ...
    <!-- Opening Skip Time -->
    <v-card>
      <v-card-text class="pb-2 caption">{{ $t('settings.player.skipOpeningTimeDescription') }}</v-card-text>
      <v-card-text>
        <v-text-field
          outlined
          hide-details
          class="mb-2"
          type="number"
          :label="$t('settings.player.skipOpeningTimeLabel')"
          :suffix="$t('common.seconds')"
