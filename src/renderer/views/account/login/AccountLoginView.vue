        ...
        <v-card flat color="transparent">
          ...
          <v-layout class="py-6 pt-2">
            <v-text-field
              ...
              class="mr-1"
              color="grey"
              :placeholder="$t('login.emailOrLoginPlaceholder')"
          ...
          <!-- Actions -->
          <v-layout>
            <v-btn v-bind="{loading}" class="mr-1" :disabled="$v.$invalid" @click="authorize">{{ $t('login.authorizeButton') }}</v-btn>
            <v-btn v-bind="{loading}" text @click="toBack">{{ $t('buttons.back') }}</v-btn>
          </v-layout>

          <v-divider class="my-6" />

          <v-layout justify-center>
            <v-btn :color="'blue darken-1'" @click="authorizeWithVK">{{ $t('login.loginViaVkButton') }}</v-btn>
          </v-layout>
    ...
    ipcRenderer.on('VK_CODE', async (event, session) => {
      try {
        ...
        if (e.response.status === 401) {
          this.$toasted.error(this.$t('login.userNotRegisteredError'))
        }
        this.loading = false
      }
    })
